...
class Enigma {
  constructor(option) {
    ...
    _defineProperty(this, "rotorsFirsts", void 0);
    ...
    this.rotors = rotors || Array.from({
      length: 3
    }).map(_ => shuf(this.rotorGenerator()));
  ...
  convertor(inputNum) {
    ...
    let phase_2 = this.rotors.map(list => toConv.bind(null, list)).reduce((result, f) => f(result), phase_1);
    let phase_3 = toConv(this.reflector, phase_2);
    let phase_4 = this.rotors.map(list => revConv.bind(null, list)).reduceRight((result, f) => f(result), phase_3);
  ...
  updateRotor() {
    for (let i = 0; i < this.rotors.length; i++) {
      if (i > 0 && this.rotors[i - 1][0] != this.rotorsFirsts[i - 1]) {
        break;
      }
